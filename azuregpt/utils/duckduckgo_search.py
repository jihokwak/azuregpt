...
def get_html(search: SearchRequest) -> SearchResponse:
    ...
    if not response.ok:
        raise Exception(
            f'Failed to fetch: {response.status_code} {response.reason}')
...
def html_to_search_results(html: str, num_results: int) -> List[SearchResult]:
    ...
    results = []
    zero_click_link = soup.select_one(
        'table:nth-of-type(2) tr td a[rel="nofollow"]')
    if zero_click_link:
        title = zero_click_link.text
        body = soup.select_one(
            'table:nth-of-type(2) tr:nth-of-type(2)').text.strip()
    ...
    web_snippets = soup.select(
        'table:nth-of-type(3) .result-snippet')[:upper_bound]
